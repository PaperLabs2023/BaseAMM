...
interface IAMM {
    ...
    function getStableLpFee()external view returns(uint);

    function getFundFee()external  view returns(uint) ;

    function getLpTokenLength() external view returns(uint);

    function getStableLpTokenLength() external view returns(uint);

    function getLpTokenList(uint _index)external view returns(address);

    function getStableLptokenList(uint _index)external view returns(address);
    
    function lpInfo(address _lpAddr) external view returns(address [2] memory);

    function getLpProfit(address _lpAddr) external  view returns(uint);

    function getLpCreatedTime(address _lpAddr) external  view returns(uint);
